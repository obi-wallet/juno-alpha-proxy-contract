...
CONTRACT_ADDRESS=juno1gsavkpywfr842hf2hcu3vs95zncu5g0ckw84twplnqupmtwx9hqqduu3zk
...
RM_HOT_WALLET_ARGS=$(jq -n --arg doomed $BAD_WALLET '{"rm_hot_wallet": {"doomed_hot_wallet": "$doomed"}}')
...
ADD_HOT_WALLET_ARGS=$(jq -n --arg newaddy $BAD_WALLET --arg reset $RESET_TIME '{"add_hot_wallet": {"new_hot_wallet": {"address":"$newaddy", "current_period_reset":$reset, "period_type":"DAILY", "period_multiple":1, "spend_limits":{"coin_limit":{"denom":"ujunox","amount":"10000"},"limit_remaining":"10000"}}}}')
echo "Arguments: $ADD_HOT_WALLET_ARGS"
$BINARY tx wasm execute $CONTRACT_ADDRESS "$ADD_HOT_WALLET_ARGS" --from=$CONTRACT_ADMIN_WALLET --node=$RPC --chain-id=$CHAIN_ID $GAS1 $GAS2 $GAS3
...
ADD2_HOT_WALLET_ARGS=$(jq -n --arg newaddy $BAD_WALLET --arg reset $RESET_TIME '{"add_hot_wallet": {"new_hot_wallet": {"address":"$newaddy", "current_period_reset":$reset, "period_type":"DAILY", "period_multiple":1, "spend_limits":{"coin_limit":{"denom":"ujunox","amount":"45000"},"limit_remaining":"45000"}}}}')
